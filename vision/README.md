...
This is the intended deployment mode for a camera mounted on a stationary traffic-light pole. Edit `zones.json`; every point is `[x, y]` normalized from `0.0` to `1.0`, measured from the video's top-left corner. The original fixed-zone warning rules are unchanged.
