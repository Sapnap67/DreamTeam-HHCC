...
if exist "%PYTHON_EXE%" goto verify_environment
...
:verify_environment
"%PYTHON_EXE%" -c "import torch, torchvision; torchvision.ops.nms(torch.tensor([[0.,0.,1.,1.]]), torch.tensor([0.9]), 0.5)" >nul 2>nul
if not errorlevel 1 goto environment_ready

echo Repairing incompatible YOLO runtime dependencies...
"%PYTHON_EXE%" -m pip install -r requirements.txt
if errorlevel 1 goto setup_failed
